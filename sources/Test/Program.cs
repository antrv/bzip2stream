...
namespace Test
{
	class Program
	{
		...
		static void DecompressPlanetFilePartially(string filename)
		{
			using (FileStream inputStream = new FileStream(filename, FileMode.Open, FileAccess.Read, FileShare.Read))
			using (BZip2Stream compressStream = new BZip2Stream(inputStream, CompressionMode.Decompress))
			using (FileStream outputStream = new FileStream(filename+".xml", FileMode.Create))
			{
		...
		static void TestCompression(string inputFile)
		{
		...
		static void Compress(string inputFile, string outputFile)
		{
			...
			using (BZip2Stream compressStream = new BZip2Stream(outputStream, BZip2CompressionLevel.Lv9))
		...
		static bool CompareFiles(string inputFile1, string inputFile2)
		{
			using (FileStream inputStream1 = new FileStream(inputFile1, FileMode.Open))
			using (FileStream inputStream2 = new FileStream(inputFile2, FileMode.Open))
			{
