...
namespace System.IO.Compression
{
    public partial class BZip2Stream : Stream
    {
        ...
        public BZip2Stream(Stream stream, BZip2CompressionLevel level, bool leaveOpen)
        {
            ...
            {
                throw new ArgumentException("level is not a valid BZip2CompressionLevel enumeration value.",
                    nameof(level));
            }
        ...
        private static BzErrorCode CheckErrorCode(BzErrorCode errorCode)
        {
            switch (errorCode)
            {
                ...
                case BzErrorCode.BZ_SEQUENCE_ERROR:
                    throw new InvalidOperationException("Invalid method invocation sequence (libbzip2.dll)");
                ...
                case BzErrorCode.BZ_DATA_ERROR_MAGIC:
                    throw new InvalidOperationException(
                        "The compressed stream doesn't begin with the right magic bytes.");
                case BzErrorCode.BZ_IO_ERROR:
                    throw new IOException("I/O error has occurred.");
            }
            ...
            object state)
        {
            if (_stream == null)
            {
                throw new ObjectDisposedException(
                    "The read operation cannot be performed because the stream is closed.");
            }
            ...
            if (Interlocked.CompareExchange(ref _activeAsyncOperation, 1, 0) != 0)
                throw new InvalidOperationException("Another asynchronous operation is active.");

            Func<int> action = () => Read(buffer, offset, count);
            BzAsyncResult asyncResult = new(action, callback, CallbackMethod, state);
            ...
            object state)
        {
            ...
            {
                throw new InvalidOperationException(
                    "The CompressionMode value was Decompress when the object was created.");
            }
